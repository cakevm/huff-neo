...
impl<'a, 'l> Compiler<'a, 'l> {
    ...
    pub fn fetch_sources(paths: Vec<PathBuf>, reader: Arc<dyn FileProvider>) -> Vec<Result<Arc<FileSource>, CompilerError>> {
        ...
        let localized_imports: Vec<String> = imports
            ...
            .map(|mut import| {
                ...
                match remapper.remap(&import) {
                    ...
                    None => {
                        import = FileSource::localize_file(&fs.path, &import).unwrap_or_default().replacen(
                            "contracts/contracts",
                            "contracts",
                            1,
                        );
    ...
    pub fn export_artifacts(artifacts: &Vec<Arc<Artifact>>, output: &OutputLocation) {
        ...
        artifacts.into_par_iter().for_each(|a| {
            ...
            let json_out = match is_file {
                true => output.0.clone(),
                false => format!("{}/{}.json", output.0, a.file.path.to_uppercase().replacen("./", "", 1)),
            };
