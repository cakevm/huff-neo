...
fn test_get_outputs_no_output() {
    ...
    let compiler: Compiler = Compiler::new(&evm_version, Arc::new(vec![]), None, None, None, None, None, false, false);
...
fn test_get_outputs_with_output() {
    ...
    let compiler: Compiler =
        Compiler::new(&evm_version, Arc::new(vec![]), Some("./test_out/".to_string()), None, None, None, None, false, false);
    let ol: OutputLocation = compiler.get_outputs();
...
fn test_transform_paths_non_huff() {
    let file_provider = FileSystemFileProvider {};
    let path_bufs: Result<Vec<PathBuf>, CompilerError> = file_provider.transform_paths(&["./ERC20.txt".to_string()]);
...
fn test_transform_paths_no_dir() {
    let file_provider = FileSystemFileProvider {};
    let path_bufs: Result<Vec<PathBuf>, CompilerError> = file_provider.transform_paths(&["./examples/random_dir/".to_string()]);
    assert!(path_bufs.is_err());
    match path_bufs {
        Err(CompilerError::FileUnpackError(e)) => {
