...
use huff_neo_utils::{abi::Abi, artifact::Artifact, error::CompilerError, prelude::EVMVersion};
...
pub fn compile(input: JsValue) -> Result<JsValue, JsValue> {
    ...
    res.into_iter().for_each(|artifact| {
        contracts.insert(
            ...
            CompilerArtifact { bytecode: artifact.bytecode.clone(), runtime: artifact.runtime.clone(), abi: artifact.abi.clone() },
        );
    });
