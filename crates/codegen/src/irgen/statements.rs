        ...
        StatementType::BuiltinFunctionCall(bf) => {
            ...
            match bf.kind {
                BuiltinFunctionKind::Codesize => {
                    let ir_macro = if let Some(m) = contract.find_macro_by_name(bf.args[0].name.as_ref().unwrap()) {
                        m
                    } else {
                        tracing::error!(
                            target: "codegen",
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::MissingMacroDefinition(bf.args[0].name.as_ref().unwrap().to_string() /* yuck */),
                            span: bf.span.clone(),
                            token: None,
                        });
                    };
                ...
                BuiltinFunctionKind::Tablesize => {
                    let ir_table = if let Some(t) = contract.find_table_by_name(bf.args[0].name.as_ref().unwrap()) {
                        t
                    } else {
                        tracing::error!(
                            target: "codegen",
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::InvalidMacroInvocation(bf.args[0].name.as_ref().unwrap().to_string() /* yuck */),
                            span: bf.span.clone(),
                            token: None,
                        });
                    };
                ...
                BuiltinFunctionKind::Tablestart => {
                    ...
                    if let Some(t) = contract.find_table_by_name(bf.args[0].name.as_ref().unwrap()) {
                        ...
                        *offset += 3;
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::InvalidMacroInvocation(bf.args[0].name.as_ref().unwrap().to_string()),
                            span: bf.span.clone(),
                            token: None,
                        });
                    }
                ...
                BuiltinFunctionKind::FunctionSignature => {
                    ...
                    if let Some(func) = contract.functions.iter().find(|f| bf.args[0].name.as_ref().unwrap().eq(&f.name)) {
                        let push_bytes = format!("{}{}", Opcode::Push4, hex::encode(func.signature));
                        *offset += push_bytes.len() / 2;
                        bytes.push((starting_offset, Bytes(push_bytes)));
                    } else if let Some(error) = contract.errors.iter().find(|e| bf.args[0].name.as_ref().unwrap().eq(&e.name)) {
                        let push_bytes = format!("{}{}", Opcode::Push4, hex::encode(error.selector));
                        *offset += push_bytes.len() / 2;
                        bytes.push((starting_offset, Bytes(push_bytes)));
                        ...
                        *offset += push_bytes.len() / 2;
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::MissingFunctionInterface(bf.args[0].name.as_ref().unwrap().to_string()),
                            span: bf.span.clone(),
                            token: None,
                        });
                    }
                ...
                BuiltinFunctionKind::EventHash => {
                    ...
                    if let Some(event) = contract.events.iter().find(|e| bf.args[0].name.as_ref().unwrap().eq(&e.name)) {
                        ...
                        *offset += push_bytes.len() / 2;
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::MissingEventInterface(bf.args[0].name.as_ref().unwrap().to_string()),
                            span: bf.span.clone(),
                            token: None,
                        });
                    }
                ...
                BuiltinFunctionKind::Error => {
                    ...
                    if let Some(error) = contract.errors.iter().find(|e| bf.args[0].name.as_ref().unwrap().eq(&e.name)) {
                        ...
                        *offset += push_bytes.len() / 2;
                        ...
                        return Err(CodegenError {
                            kind: CodegenErrorKind::MissingErrorDefinition(bf.args[0].name.as_ref().unwrap().to_string()),
                            span: bf.span.clone(),
                            token: None,
                        });
                    }
