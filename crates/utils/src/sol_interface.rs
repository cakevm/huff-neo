        ...
        if let Some(a) = &artifact.abi {
            ...
            a.events.iter().for_each(|(_, f)| {
                defs.push(format!(
                    ...
                    f.inputs
                        .iter()
                        .map(|i| { format!("{}{}", i.kind, if i.indexed { String::from(" indexed") } else { String::default() }) })
                        .collect::<Vec<_>>()
                        .join(", "),
                ));
            });
            a.errors.iter().for_each(|(_, e)| {
                defs.push(format!(
                    ...
                    e.inputs
                        .iter()
                        .map(|i| format!("{}{}", i.kind, if i.kind.is_memory_type() { " memory" } else { "" }))
                        .collect::<Vec<_>>()
                        .join(", "),
                ));
            });
                ...
                defs.push(format!(
                    ...
                    if f.outputs.is_empty() {
                        ...
                        format!(
                            ...
                            f.outputs
                                .iter()
                                .map(|o| format!("{}{}", o.kind, if o.kind.is_memory_type() { " memory" } else { "" }))
                                .collect::<Vec<_>>()
                                .join(", ")
                        )
                    },
