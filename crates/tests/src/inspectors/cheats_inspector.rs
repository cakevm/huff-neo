...
lazy_static! {
    pub static ref CHEATS_ADDR: Address = Address::from_str("00000000000000000000000000000000bEefbabe").unwrap();
}
...
where
    DB: Database,
{
    fn log(&mut self, _: &mut EVMData<'_, DB>, _: &revm::primitives::B160, _: &[revm::primitives::B256], _: &Bytes) {
        unimplemented!()
    }

    fn call(&mut self, _data: &mut EVMData<'_, DB>, inputs: &mut CallInputs, _is_static: bool) -> (InstructionResult, Gas, Bytes) {
    ...
    fn create(&mut self, _data: &mut EVMData<'_, DB>, call: &mut CreateInputs) -> (InstructionResult, Option<B160>, Gas, Bytes) {
