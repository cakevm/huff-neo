...
impl Codegen {
    ...
    pub fn generate_main_bytecode_with_sourcemap(
        ...
        Codegen::gen_table_bytecode(evm_version, &contract, bytecode_res)
    }
    ...
    pub fn update_table_size(evm_version: &EVMVersion, contract: &Contract) -> Result<Contract, CodegenError> {
        ...
        for table_definition in contract.tables.iter() {
            ...
            let byte_code = Codegen::gen_table_bytecode_builtin(evm_version, contract, table_definition)?;
    ...
    pub fn gen_table_bytecode(
        ...
        res.utilized_tables.iter().try_for_each(|jt| {
            ...
            if jt.kind == TableKind::CodeTable {
                let table_code = Codegen::gen_table_bytecode_builtin(evm_version, contract, jt)?;
