...
fn test_missing_constructor() {
    ...
    // Full source
    let full_source = FileSource { source: Some(source.to_string()), path: "".to_string(), access: None, dependencies: vec![] };

    let evm_version = EVMVersion::default();

    // Instantiate a new compiler
    let compiler = Compiler::new(&evm_version, Arc::new(vec![]), None, None, None, None, None, false, false);
    ...
    match compiler.gen_artifact(Arc::clone(&arc_source)) {
        Ok(artifact) => {
            ...
            assert_eq!(artifact.bytecode, "60188060093d393df35f3560e01c806340c10f1914610010575b6004355f602435".to_string());
        }
        _ => panic!("moose"),
    }
}
...
fn test_missing_constructor_with_inputs() {
    ...
    // Instantiate a new compiler
    let compiler = Compiler::new(&evm_version, Arc::new(vec![]), None, None, None, Some(vec!["0".to_string()]), None, false, false);
